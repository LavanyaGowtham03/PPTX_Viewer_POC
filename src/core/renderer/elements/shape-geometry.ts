...
const attrs = (element: SVGElement, values: Record<string, string | number>): void => {
...
const buildOval = (W: number, H: number, paint: ShapePaint): SVGElement => {
...
const buildTriangle = (W: number, H: number, paint: ShapePaint): SVGElement => {
 ...
 */
const buildBlockArrow = (W: number, H: number, paint: ShapePaint): SVGElement => {
  const polygon = document.createElementNS(SVG_NS, 'polygon');
  attrs(polygon, {
    points: [
...
const buildFlowchartTerminator = (
  W: number,
  H: number,
  paint: ShapePaint
): SVGElement => {
 ...
 */
export function renderUnknownShapeFallback(
  wrapper: HTMLElement,
  box: FallbackBox
): HTMLElement {
