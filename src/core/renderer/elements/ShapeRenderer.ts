 ...
 */
export function renderShape(el: ShapeElement, scale: number): HTMLElement {
 ...
 */
export function renderShapeFallback(el: UnknownElement, scale: number): HTMLElement {
  const wrapper = document.createElement('div');
  wrapper.className =
  ...
  console.warn(
    `[PptxViewer] Unknown shapeType "${el.shapeType ?? '(missing)'}" (id: ${el.id}) — ` +
