 ...
 */
export function renderSlide(slide, pres, scale, suppressAnimations = false) {
    ...
    for (const element of slide.elements) {
        try {
            ...
            console.warn(`[PptxViewer] Failed to render element "${element.id}" on slide ` +
 ...
 */
export function applyPresentationDimensions(root, pres) {
    ...
    root.style.setProperty('--pptx-thumbnail-height', `${Math.round(THUMBNAIL_WIDTH * (pres.height / pres.width))}px`);
 ...
 */
export function computeScale(availableWidth, pres) {
