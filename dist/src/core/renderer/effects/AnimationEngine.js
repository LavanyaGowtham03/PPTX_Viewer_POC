 ...
 */
export function applyAnimation(el, anim) {
    if (anim === undefined) {
    ...
    const type = KNOWN_ANIMATIONS.has(String(anim.type))
        ? anim.type
        : 'fadeIn';
    if (String(anim.type) !== type) {
        console.warn(`[PptxViewer] Unknown animation type "${String(anim.type)}" — falling back to fadeIn.`);
    }
 ...
 */
export function replayAnimations(slideEl) {
    const animated = Array.from(slideEl.querySelectorAll(`.${ANIMATED_CLASS}`));
    if (animated.length === 0) {
        return; // nothing to replay — safe no-op
    }
    const saved = [];
    for (const el of animated) {
        const animClasses = Array.from(el.classList).filter((cls) => cls.startsWith(ANIM_CLASS_PREFIX));
 ...
 */
export function setAnimationsEnabled(root, enabled) {
