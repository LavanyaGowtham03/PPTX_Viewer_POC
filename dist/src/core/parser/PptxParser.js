...
export const INVALID_FILE_MESSAGE = 'Invalid file. Please select a valid PPTX JSON file.';
// ─────────────────────────────────────────────────────────────────────────────
...
const KNOWN_TRANSITIONS = new Set(['fade', 'push', 'wipe', 'zoom']);
...
const normalizeAnimation = (raw) => {
    const rec = asRecord(raw);
    if (!rec) {
        return undefined; // no animation — element simply renders visible
    }
    const typeRaw = asString(rec['type']);
    const type = typeRaw !== null && KNOWN_ANIMATIONS.has(typeRaw) ? typeRaw : 'fadeIn';
    if (typeRaw !== null && typeRaw !== type) {
        console.warn(`[PptxViewer] Unknown animation type "${typeRaw}" — falling back to fadeIn.`);
    }
    return {
        type,
...
const normalizeTransition = (raw) => {
    ...
    const typeRaw = asString(rec['type']);
    const type = typeRaw !== null && KNOWN_TRANSITIONS.has(typeRaw) ? typeRaw : 'fade';
    if (typeRaw !== null && typeRaw !== type) {
        console.warn(`[PptxViewer] Unknown transition type "${typeRaw}" — falling back to fade.`);
    }
    const directionRaw = asString(rec['direction']);
    const direction = directionRaw !== null && KNOWN_DIRECTIONS.has(directionRaw)
        ? directionRaw
        : undefined;
...
const makeUnknown = (base, shapeTypeRaw) => ({
    ...base,
    ...(shapeTypeRaw !== null ? { shapeType: shapeTypeRaw } : {}),
    _unknown: true,
});
const normalizePicture = (rec) => {
...
const normalizeTableCell = (raw) => {
    ...
    return {
        ...(asString(rec['text']) !== null ? { text: asString(rec['text']) } : {}),
...
const normalizeElement = (raw) => {
    ...
    if (rec !== null && type === 'textbox')
        return normalizeTextBox(rec);
    if (rec !== null && type === 'shape')
        return normalizeShape(rec);
    if (rec !== null && type === 'picture')
        return normalizePicture(rec);
    if (rec !== null && type === 'table')
        return normalizeTable(rec);
    ...
    return makeUnknown(rec !== null ? normalizeBase(rec) : emptyBase(), asString(rec?.['shapeType']));
};
...
// Slide normalisation
// ─────────────────────────────────────────────────────────────────────────────
const normalizeSlide = (raw, index) => {
    const rec = asRecord(raw) ?? {};
    return {
