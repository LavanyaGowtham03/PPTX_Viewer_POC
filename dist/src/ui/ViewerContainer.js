...
import { applyPresentationDimensions, computeScale, } from '../core/renderer/slide-renderer';
import { setAnimationsEnabled, setTransitionsEnabled, transitionTo, } from '../core/renderer/effects';
 ...
 */
export function renderFullViewer(container, data) {
    ...
    const goToSlide = (index) => {
        ...
        void transitionTo(activeSlideEl, next, incoming, root)
            .then(() => {
            currentIndex = clamped;
            activeSlideEl = next;
            updateInfo();
    ...
    const loadViaSearch = async (file) => {
        try {
